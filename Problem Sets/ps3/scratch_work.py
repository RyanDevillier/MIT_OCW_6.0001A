...
hand = {'l': 1, 'd': 2, '*': 1, 'o': 1, 'u': 1, 't': 1}
substitute_letter = input('Do you want to substitute a letter in the hand?: ')

if substitute_letter.lower() == 'yes':
    letter_to_replace = input('Which letter would you like to replace: ')
    if letter_to_replace.lower() in hand:
        new_letter = random.choice(list(SCRABBLE_LETTER_VALUES.keys()))
    
# now take the old key and replace it with new_letter
hand[new_letter] = hand[letter_to_replace]
del hand[letter_to_replace]
